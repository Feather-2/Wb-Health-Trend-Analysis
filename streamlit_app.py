        ...
        if selected_category:
            ...
            with subtab1:
                ...
                if forecast_fig:
                    ...
                    with st.expander("热搜数量趋势预测图表说明"):
                        st.write("""
                        “热搜数量趋势预测”图表是通过 `plot_prophet_forecast` 函数生成的交互式可视化图表，利用 Prophet 时间序列预测模型展示特定类别热搜数量的月度趋势，并预测未来三个月的热搜数量。以下是对该图表的详细说明，包括其结构、内容和意义：
                    
                        ---
                    
                        ### 图表结构
                        该图表是一个组合图，包含以下元素：
                        1. **历史数据点**：显示实际记录的每月热搜数量。
                        2. **趋势线**：基于 Prophet 模型计算的长期趋势。
                        3. **预测曲线**：展示历史数据和未来三个月的预测值。
                        4. **倍数曲线**：以历史基准值为参考，显示热搜数量的相对变化倍数。
                        ...
                        #### 1. 横轴（X轴）
                        - 表示时间，按月度显示（格式为 `%Y-%m`），覆盖历史数据范围并延伸到未来三个月。
                        - 时间范围自动调整，确保包含所有历史数据点和预测区间。
                    
                        #### 2. 左侧纵轴（Y轴，热搜数量）
                        - 表示每月的热搜数量，单位为条数。
                        - 显示历史数据点（红色散点）、Prophet 预测值（蓝色曲线）和趋势线（绿色虚线）。
                    
                        #### 3. 右侧纵轴（Y轴，基准倍数）
                        - 表示热搜数量相对于历史基准值的倍数，格式为小数（例如 2.0 表示两倍）。
                        - 用紫色曲线绘制，反映热搜数量的相对变化幅度。
                        ...
                        - **历史数据点**：
                          ...
                          - 悬停时显示日期和具体数量。
                        - **Prophet 预测值**：
                          - 蓝色实线表示历史和预测的热搜数量，包括置信区间（浅蓝色阴影）。
                          - 未来三个月的预测值基于历史数据的季节性和趋势。
                        - **趋势线**：
                          - 绿色虚线表示 Prophet 模型提取的长期趋势，突出热搜数量的整体增长或下降方向。
                        - **倍数曲线**：
                          - 紫色实线表示热搜数量相对于历史基准值的倍数。
                          - 基准值根据数据时间范围计算：
                            - 如果数据早于 2020 年，使用 2020 年之前数据的平均值。
                            - 如果数据都在 2020 年之后，使用全部数据的平均值。
                          - 悬停时显示日期和倍数值。
                    
                        #### 5. 交互功能
                        - 鼠标悬停时显示详细信息，包括日期、实际值（若有）、预测值、趋势值和倍数。
                        - 支持缩放和平移，便于查看特定时间段。
                        ...
                        #### 1. 历史热搜数量的月度变化
                        - 红色数据点展示了每个月热搜数量的实际情况，帮助用户识别高峰期和低谷期。
                        - 例如，可能显示冬季月份（如 1 月）热搜数量较高，夏季（如 7 月）较低。
                    
                        #### 2. 长期趋势
                        - 绿色趋势线揭示热搜数量的整体走向，例如是否逐年增加、稳定或下降。
                        - 这有助于判断该类别话题的关注度是否在长期内发生显著变化。
                    
                        #### 3. 未来三个月的预测
                        - 蓝色预测曲线和置信区间预测了未来三个月的热搜数量。
                        - 置信区间反映预测的不确定性，区间越宽表示预测可靠性越低。
                    
                        #### 4. 相对变化幅度
                        - 紫色倍数曲线量化了热搜数量相对于历史基准的增长或减少倍数。
                        - 例如，倍数为 2.5 表示该月热搜数量是基准值的 2.5 倍，表明话题热度显著提升。
                    
                        ---
                    
                        ### 图表示例分析（假设数据）
                        假设选择了“咳嗽”类别，图表可能显示：
                        - **历史数据**：2024 年 12 月有 50 条热搜，2025 年 1 月增至 80 条，显示冬季高峰。
                        - **趋势线**：绿色虚线缓慢上升，表明“咳嗽”相关热搜的关注度逐年增加。
                        - **预测值**：2025 年 4-6 月预测值为 40-50 条，置信区间为 30-60 条，反映春季热度可能下降。
                        - **倍数曲线**：2025 年 1 月倍数为 2.0（基准值为 40），表示热度是平均水平的两倍。
                    
                        ---
                    
                        ### 使用场景
                        1. **季节性分析**：识别热搜数量的季节性模式（如冬季疾病话题高发）。
                        2. **未来规划**：根据预测值提前准备相关内容或资源。
                        3. **影响力评估**：结合倍数曲线判断话题热度的相对重要性。
                    
                        ---
                    
                        ### 限制与注意事项
                        - **数据量要求**：需要至少 3 个月的数据才能进行预测，否则会显示警告。
                        - **预测准确性**：受历史数据质量和外部事件影响，预测可能存在偏差。
                        - **基准值局限**：基准值的选择（2020 年前或全部数据）可能影响倍数曲线的解释。
                    
                        ---
                    
                        ### 总结
                        “热搜数量趋势预测”图表通过历史数据、趋势线、预测值和倍数曲线，全面展示了特定类别热搜数量的过去表现和未来走势。它结合了直观的视觉效果和 Prophet 模型的预测能力，为用户提供了分析话题热度变化和制定决策的强大工具。
                        """)
                else:
            ...
            with subtab2:
                ...
                with st.expander("相关热搜时间线图表说明"):
                    st.write("""
                    “相关热搜时间线”图表是通过 `plot_timeline_gantt` 函数生成的甘特图（Gantt Chart）形式的交互式可视化图表，用于展示特定类别热搜的时间分布、持续时间、排名变化趋势以及显著变化的时间段。以下是对该图表的详细说明，包括其结构、内容和意义：
                
                    ---
                
                    ### 图表结构
                    该图表由两部分组成，垂直排列为两个子图：
                    1. **上半部分（主图）**：展示热搜事件的时间线，横轴为时间，纵轴为热搜的相对位置（不显示具体数值，而是用于区分不同的热搜事件）。
                    2. **下半部分（辅助图）**：展示热搜数量的变化率和对数趋势，横轴与主图共享时间轴，纵轴分别表示变化率（左侧）和对数趋势（右侧）。
                    ...
                    #### 1. 主图（热搜时间线）
                    - **横轴（X轴）**：表示时间，覆盖所选类别热搜的完整时间范围（从最早到最晚的热搜时间），刻度通常以日期和小时显示（格式为 `%Y/%m/%d %H:%M`）。
                    - **纵轴（Y轴）**：不显示具体数值，而是用来表示不同热搜事件的相对位置。为了避免重叠，代码通过时间窗口（默认72小时）动态调整热搜的纵向位置：
                      - 如果两个热搜的开始时间间隔超过72小时，它们可能被放置在较低的纵向位置（重用空间）。
                      - 如果时间重叠或接近，则纵向位置递增，形成分层效果。
                    - **热搜条（Bars）**：
                      - 每个热搜事件用一条水平线段表示，线的起点是热搜的开始时间（`date`），终点是开始时间加上持续时间（`date + duration_mins`）。
                      - 线条颜色根据热搜的最高排名（`rank`）映射到 `Viridis` 颜色序列，排名越高（数值越小），颜色越深。
                      - 线条粗细固定（宽度为10），便于直观区分。
                    - **端点标记**：每个热搜的开始时间点用圆形标记表示，颜色与对应的线条一致，带有白色边框。
                    - **平滑连接线**：用灰色细线（`lightgray`）连接平滑后的纵向位置（`y_positions_smooth`），展示热搜活动的连续性。
                    - **交互功能**：鼠标悬停时显示详细信息，包括标题、开始时间、持续时间和最高排名。
                
                    #### 2. 辅助图（变化率和趋势分析）
                    - **横轴（X轴）**：与主图共享时间轴，确保上下图时间对齐。
                    - **左侧纵轴（Y轴，变化率）**：
                      - 表示热搜数量的平滑变化率（`normalized_slopes`），通过计算平滑后的纵向位置（`y_positions_smooth`）随时间的变化斜率（`dy/dx`）得出。
                      - 变化率经过归一化处理，正负值表示热搜活动增加或减少的趋势。
                      - 用灰色半透明线条（`rgba(100,100,100,0.5)`）绘制。
                    - **右侧纵轴（Y轴，对数趋势）**：
                      - 表示基于变化率计算的累积对数趋势（`normalized_log_trend`），反映热搜活动的长期增长或衰减趋势。
                      - 用蓝色虚线（`dash='dot'`）绘制，归一化到0-1区间。
                    - **显著变化区间**：
                      - 通过 `find_significant_periods` 函数识别变化率超出阈值（1.5倍标准差）的显著时间段。
                      - 用红色透明矩形（`fillcolor="red", opacity=0.2`）高亮这些区间。
                      - 每个显著区间标注变化率的平均值（`mean_slope`），显示在矩形顶部。
                    - **交互功能**：鼠标悬停时显示变化率和趋势的具体数值。
                    ...
                    #### 1. 热搜的时间分布和持续时间
                    - 主图通过水平线段直观展示了每个热搜的上榜时间和持续时长。线段越长，表示热搜在榜时间越久；颜色越深，表示排名越高（更靠前）。
                    - 通过纵向位置的排列，可以观察热搜事件是否集中在某些时间段。例如，如果某个时间段内纵向位置密集，说明该时段热搜活动频繁。
                
                    #### 2. 热搜活动的趋势
                    - 平滑连接线和辅助图中的变化率曲线反映了热搜数量随时间的变化趋势：
                      - **变化率正值**：热搜数量增加，活动变活跃。
                      - **变化率负值**：热搜数量减少，活动趋于平静。
                    - 对数趋势线进一步揭示了长期趋势，例如热搜活动是否呈现指数增长、稳定或衰减。
                
                    #### 3. 显著变化的时间段
                    - 红色高亮区域标示了热搜活动发生显著变化的时间段（例如突然激增或骤减）。这些区域通常对应于重大事件或话题热度的集中爆发。
                    - 标注的变化率数值量化了这些变化的强度，帮助用户理解哪些时间段的波动最为剧烈。
                
                    #### 4. 热搜排名的重要性
                    - 颜色编码反映了热搜的最高排名，结合时间线可以分析高排名热搜（深色）是否集中在特定时期，或者是否与显著变化区间相关。
                
                    ---
                
                    ### 图表示例分析（假设数据）
                    假设选择了“咳嗽”类别，图表可能显示：
                    - **2025年1月**：多条深色长线段密集排列，变化率曲线急剧上升并出现红色高亮区，可能表明“咳嗽”相关话题因冬季流感高峰而热度激增。
                    - **2025年3月**：线段稀疏，变化率趋于平稳，对数趋势下降，可能表示相关话题热度消退。
                    - **显著区间**：1月15日-1月20日，变化率标注为+0.45，表明这段时间内热搜数量快速增加。
                
                    ---
                
                    ### 使用场景
                    1. **事件追踪**：识别某类别热搜的高峰期和低谷期，分析背后可能的事件驱动因素。
                    2. **趋势预测**：通过变化率和对数趋势，推测未来热搜活动的潜在走向。
                    3. **影响力评估**：结合排名颜色和持续时间，评估哪些热搜更具影响力。
                
                    ---
                
                    ### 总结
                    “相关热搜时间线”图表综合展示了时间维度上的热搜分布、持续时间、排名重要性以及活动变化趋势。通过交互式功能，用户可以深入探索具体热搜的细节，并结合显著变化区间分析关键时间点的异常波动。这对于理解社交媒体话题的动态演变和潜在社会关注点具有重要价值。
                    """)
